...
def summarize_orders_for_coin(coin, price_lookup=None):
    coin_df = df_spot[df_spot[crypto_col_spot] == coin]

    buy_trades = extract_side(coin_df, "Buy", crypto_col_spot, "Quantity",
                              "Net Amount Paid/Received by the user(in base currency)",
                              "Trade Completion time")
    sell_trades = extract_side(coin_df, "Sell", crypto_col_spot, "Quantity",
                               "Net Amount Paid/Received by the user(in base currency)",
                               "Trade Completion time")
...
def summarize_all_coins(price_lookup=None):
    coins = df_spot[crypto_col_spot].dropna().unique()
    all_summary = {}
    for coin in coins:
