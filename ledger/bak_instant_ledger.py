...
df_spot = pd.read_excel(file_path, sheet_name="Instant Orders", header=8, engine='openpyxl')
df_spot.columns = df_spot.columns.str.strip()
crypto_col_instant = [col for col in df_spot.columns if "Crypto" in col][0]


sol_df_spot = df_spot[df_spot[crypto_col_instant] == 'SOL']
...
def summarize_orders():
    spot_result = {
        "buy": extract_side(sol_df_spot, "Buy", "Crypto", "Quantity",
                            "Net Amount Paid/Received by the user(in INR)",
                            "Trade Completion time"),
        "sell": extract_side(sol_df_spot, "Sell", "Crypto", "Quantity",
                            "Net Amount Paid/Received by the user(in INR)",
                            "Trade Completion time")
    }
